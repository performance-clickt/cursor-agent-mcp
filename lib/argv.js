 ...
 *  - optionally injects `--print --output-format <fmt> --trust` (when print is
 *    true) — `--trust` skips the interactive "Workspace Trust Required" prompt
 *    that would otherwise block a headless `--print` run (HM-582),
 ...
 *  - inserts `--model <model>` from per-call `model` OR the CURSOR_AGENT_MODEL
 *    env, unless a model flag is already present or no model is effective
 *    (the real cursor-agent CLI has no `-m` alias — see HM-581),
...
export function buildArgv(
  ...
  // the first positional, silently dropping a trailing --model/-f (and the model
  // override with it). Keeping the flags up front avoids that.
  return [
    ...(print ? ['--print', '--output-format', output_format, '--trust'] : []),
...
/**
 * Normalize the raw `cursor_agent_chat` tool input into the flat object passed
 * to runCursorAgent(). Reproduces the inline normalization previously in the
 * chat handler in server.js exactly, including its quirks:
 *
 *  - `prompt` resolves to the top-level `prompt` when that key is present on
 *    `args` (even if its value is undefined/null), else falls back to
 *    `args.arguments.prompt` (when `args.arguments` is an object).
 *  - the rest of `flat` is a spread of `args.arguments` (when it is an object)
 *    else a spread of `args` itself — NOT a spread of `args` followed by
 *    `args.arguments`. So when `args.arguments` is an object, top-level sibling
 *    fields of `args` (other than the resolved `prompt`) are dropped.
 *  - the resolved `prompt` is then set on top, so it always wins over
 *    whatever `prompt` field the spread contributed.
 *
 * @param {*} args
 * @returns {object} the flat object to pass to runCursorAgent()
 */
export function normalizeChatArgs(args) {
  const prompt =
    (args && typeof args === 'object' && 'prompt' in args ? args.prompt : undefined) ??
    (args && typeof args === 'object' && args.arguments && typeof args.arguments === 'object'
      ? args.arguments.prompt
      : undefined);
...
/**
 * Format a single-line run-metadata block for an opt-in "make the route/cost
 * observable" note appended to a tool result (HM-570). Pure and side-effect
 * free so it can be unit-tested without a cursor-agent binary.
 *
 * An empty/whitespace/missing model renders as "(default)" (no model resolved,
 * so cursor-agent's own default applies). A null/undefined exitCode renders as
 * "n/a" (branches like a spawn failure never produce one); exit code 0 renders
 * as "0", so the null check must be explicit rather than falsy.
 *
 * @param {object} opts
 * @param {string}        [opts.model]       resolved model (empty => "(default)")
 * @param {number}        opts.durationMs    wall-clock run duration in ms
 * @param {number|null}   [opts.exitCode]    process exit code (null/undefined => "n/a")
 * @param {number}        opts.bytes         stdout bytes accumulated
 * @param {boolean}       opts.truncated     whether stdout hit the byte cap
 * @returns {string} e.g. `[run: model=glm-5.2 duration=1234ms exit=0 bytes=42 truncated=false]`
 */
export function formatRunMetadata({ model, durationMs, exitCode, bytes, truncated }) {
  const modelStr = (model && String(model).trim()) || '(default)';
  const exitStr = exitCode === null || exitCode === undefined ? 'n/a' : String(exitCode);
  return `[run: model=${modelStr} duration=${durationMs}ms exit=${exitStr} bytes=${bytes} truncated=${Boolean(truncated)}]`;
}

/**
 * Parse `cursor-agent --list-models` output into a list of model ids.
 *
 * The command prints an "Available models" header, then lines of the form
 * `<id> - <Label>` (e.g. `glm-5.2-high - GLM 5.2`). We take the token before the
 * first " - " on each line and keep the ones that look like ids (no spaces).
 * Used by the HM-557 reachability preflight.
 *
 * @param {string} output  raw stdout from `cursor-agent --list-models`
 * @returns {string[]} the parsed model ids
 */
