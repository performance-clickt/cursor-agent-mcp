...
//
// The functions here reproduce the CURRENT runtime behavior of server.js
// exactly. Where behavior is intentionally preserved for a follow-up issue,
// it is called out inline (see HM-558 for argv ordering).
 ...
 *  - passes user/extra args through untouched,
 *  - appends `-f` from per-call `force` OR the CURSOR_AGENT_FORCE env, unless a
 *    force flag is already present or force is not effective,
 *  - appends `-m <model>` from per-call `model` OR the CURSOR_AGENT_MODEL env,
...
export function buildArgv(
  ...
  // HM-558 will change this ordering: force/model flags are currently emitted
  // AFTER the positional prompt (i.e. after userArgs). Do not reorder here.
  return [
